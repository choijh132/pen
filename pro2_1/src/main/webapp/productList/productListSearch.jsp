...
<%@ page import="java.util.List" %>
 <%@ include file="/productList/mainTopSearch.jsp"%> 
<style>
...
<% 
    ...
    list = dao.getProductListSearch(category, code , order, pMin, pMax, startCount, endCount, search);
	...
	totalCount = dao.getProductListSearchCount(category, code , order, pMin, pMax, search);
%>

<table border="0" width="1200" >
	<tr>
		<td width="100"></td>
		<td><a href="productListSearch.jsp?pageNum=<%=pageNum%>&category=<%=category%>&code=<%=code%>&order=reviewcount&price1=<%=price1%>&price2=<%=price2%>&search=<%=search%>">리뷰순</a></td>
		<td width="20"></td>
		<td><a href="productListSearch.jsp?pageNum=<%=pageNum%>&category=<%=category%>&code=<%=code%>&order=salecount&price1=<%=price1%>&price2=<%=price2%>&search=<%=search%>">판매순</a></td>
		<td width="400"></td>
		<td> 가격</td>
		<td>
		<form action="productListSearch.jsp" onsubmit="return priceCheck()" >
	...
	<tr>
		<td align="center" colspan="2">
			<% int pageCount=0;
				...
				if (startPage > 10) {    %>
		        <a href="productListSearch.jsp?pageNum=<%= startPage - 10 %>&category=<%=category%>&code=<%=code%>&order=<%=order%>&price1=<%=price1%>&price2=<%=price2%>">[이전]</a>
				<%}
				
				for(int i = startPage ; i <= endPage ; i++){%>
					    <a href="productListSearch.jsp?pageNum=<%=i %>&category=<%=category%>&code=<%=code%>&order=<%=order %>&search=<%=search%>&price1=<%=price1%>&price2=<%=price2%>">[<%=i %>]</a>
				
				<%}
		        if (endPage < pageCount) {  %>
  				<a href="productListSearch.jsp?pageNum=<%= startPage + 10 %>&category=<%=category%>&code=<%=code%>&order=<%=order%>&price1=<%=price1%>&price2=<%=price2%>">[다음]</a>
				<%}%>
			</td> 
		</tr>
		<tr>
